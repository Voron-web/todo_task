...
const Header = () => {
	const { theme, changeTheme } = useThemeSet();
	...
	const style = {
		...
		svg: css({
			...
			cursor: "pointer",
			"&:hover": {
				filter: `drop-shadow(0 0 1px ${theme.accentColor})`,
			},
		}),
		colorBtn: css({
			margin: "0 auto",
	...
	return (
		<>
			<div css={style.header}>
				<div className="wrapper">
					<div css={style.headerContent}>
						...
						<div>
							<svg onClick={toggleModal} css={style.svg} fill="#000000" id="Layer_1" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512">
								<g>
			...
			{isModalOpen ? (
				<ModalWindow title={"Color theme"} isShow={isShowModal} closeModal={() => toggleModal()}>
					{themeList.map((item, index) => (
						<div onClick={() => setTheme(item.value)} key={index} css={style.colorBtn}>
							{item.name}
						</div>
					))}
				</ModalWindow>
